    ...
    if (screen === "events") {
      ...
      if (key.upArrow) {
        ...
        if (selectedIndex === 0) {
          ...
          // TODO(LET-XXXX): Implement edit functionality for existing matchers
          // Implementation steps:
          //   1. Add new screen states to Screen type: "edit-matcher" and "edit-command"
          //   2. Add state: const [editMatcherIndex, setEditMatcherIndex] = useState(-1)
          //   3. When Enter is pressed on existing matcher:
          //      - setEditMatcherIndex(selectedIndex - 1)
          //      - Pre-populate newMatcher and newCommand with existing values
          //      - setScreen("edit-matcher")
          //   4. In edit screens, call updateHookMatcher() from hooks/writer.ts instead of addHookMatcher()
          //   5. Skip save-location screen (use existing matcher's source)
          //   6. Update help text to show "Enter to edit" instead of "Enter to select"
          // Reference the add flow (lines 161-189) as a template
        }
