...
export default function App({
  ...
  // TODO(LET-XXXX): Convert userCancelledRef to state for proper React dependency tracking
  // Current workaround: refs aren't in dependency arrays, causing potential stuck queues
  // Recommended fix:
  //   1. Replace `const userCancelledRef = useRef(false)` with `const [userCancelled, setUserCancelled] = useState(false)`
  //   2. Update all reads from `userCancelledRef.current` to `userCancelled`
  //   3. Update all writes from `userCancelledRef.current = X` to `setUserCancelled(X)`
  //   4. Add `userCancelled` to dequeue effect's dependency array
  //   5. Remove any dequeueEpoch workarounds if present
  // This ensures the dequeue effect triggers when cancellation state changes
  ...
  useEffect(() => {
    if (
      ...
      if (
        (agentState as { last_stop_reason?: string } | null)
          ?.last_stop_reason === "requires_approval"
      ) {
        getClient()
          .then(async (client) => {
    ...
    async (
      ...
      try {
        ...
        const isPendingApprovalError =
          ...
          (("error_code" in e.error &&
            (e.error as { error_code?: unknown }).error_code ===
              "PENDING_APPROVAL") ||
          ...
          try {
            ...
            const runIdToCancel = (() => {
              ...
              if (nestedError && typeof nestedError === "object") {
                const nestedRunId = (nestedError as { run_id?: unknown })
                  .run_id;
                if (typeof nestedRunId === "string" && nestedRunId)
                  return nestedRunId;

                const nestedId = (
                  nestedError as { pending_request_id?: unknown }
                ).pending_request_id;
                if (typeof nestedId === "string" && nestedId) return nestedId;
              }
            ...
            try {
              ...
              const { pendingApprovals: approvalsAfterCancel } =
                await getResumeData(client, agentAfterCancel);
...
`;
  ...
  const handleModelSelect = useCallback(
    ...
    [agentId, refreshDerived, currentToolset, currentSystemPromptId, withCommandLock, isAgentBusy],
  );
  ...
  useEffect(() => {
    if (
      ...
      const headerMessage = resumedExistingConversation
        ? `Resuming conversation with **${agentName}** (last used in ${shortCwd})`
        : continueSession
          ? `Starting new conversation with **${agentName}**`
