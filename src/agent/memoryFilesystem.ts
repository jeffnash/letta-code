...
/**
 * Validate and sanitize a block label to prevent path traversal attacks.
 * Rejects labels that are absolute paths, contain ".." segments, or would escape the target directory.
...
export function validateBlockLabel(label: string, targetDir: string): string {
  ...
  // Normalize the label (convert backslashes to forward slashes, collapse redundant separators)
  const normalizedLabel = normalize(label).replace(/\\/g, "/");

  // Construct the full path and verify it stays within the target directory
...
async function writeMemoryFile(dir: string, label: string, content: string) {
  const validatedLabel = validateBlockLabel(label, dir);
  const filePath = join(dir, `${validatedLabel}.md`);
